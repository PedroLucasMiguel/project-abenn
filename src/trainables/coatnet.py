...
class TrainableCoatNetBaseline(TrainerFramework):
  def __init__(self, dataset_name: str) -> None:
    self.n_classes = len(os.listdir(f"../datasets/{dataset_name}"))
    self.trainable_model = CoatNetGradCAM(timm.create_model('coatnet_0_rw_224.sw_in1k', pretrained=True), n_classes=self.n_classes)
...
class TrainableCoatNetABNCFGAP(TrainerFramework):
  def __init__(self, dataset_name: str) -> None:
    self.n_classes = len(os.listdir(f"../datasets/{dataset_name}"))
    self.baseline = timm.create_model('coatnet_0_rw_224.sw_in1k', pretrained=True)
    self.trainable_model = CoatNetABNCFGAP(baseline_model=self.baseline, n_classes=self.n_classes)
